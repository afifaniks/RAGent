...
from swebench.harness.constants import (FAIL_TO_PASS, KEY_INSTANCE_ID,
                                        ...
                                        PASS_TO_PASS, USE_X86,
                                        SWEbenchInstance)
from swebench.harness.docker_build import build_env_images
from swebench.harness.run_evaluation import (get_dataset_from_preds,
                                             run_instance)
from swebench.harness.test_spec import (TestSpec, make_env_script_list,
...
def run_tests(
    ...
    for instance in instances:
        ...
        # NOTE: COMMENT THE NEXT BLOCK FOR REPRODUCTION TESTS. IT IS ONLY FOR REGRESSION TESTS
        if not instance_test_dict.get(instance["instance_id"]):
            print(f"No tests found for instance {instance['instance_id']}")
            continue
