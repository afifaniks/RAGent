...
def process_loc(loc, args, swe_bench_data, prev_o, write_lock=None):
    ...
    for i, pred_file in enumerate(pred_files):
        content = None
        if pred_file in ["seaborn/_core.py"]:
            print("WARNING: seaborn/_core.py skipped due to mismatch issue.")
            continue
...
def repair(args):
    ...
    print(f"loaded {len(prev_o)} previous outputs")
    # print(prev_o)
    ...
    if args.num_threads == 1:
        if args.target_id is not None:
            locs = [loc for loc in locs if loc["instance_id"] == args.target_id]
