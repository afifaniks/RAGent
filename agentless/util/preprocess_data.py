        ...
        for loc in model_pred_locs.splitlines():
            ...
            if loc.startswith("class: ") and "." not in loc:
                ...
                if "." in loc:
                    ...
                    relevant_class = [  # relevant_class = [{'file': 'astropy/nddata/mixins/ndarithmetic.py', 'name': 'NDArithmeticMixin', 'start_line': 103, 'end_line': 750,
                        clazz
                        for clazz in classes
                    ...
                    if len(relevant_class) == 0:
                        ...
                        relevant_method = [  # relevant methods should have some output [{'name': '_arithmetic_mask', 'start_line': 485, 'end_line': 527}]
                            method
    ...
    for filepath in filepaths:
        ...
        for file_content in files:
            if file_content[0] == filepath:
                ...
                break
        if content is None and ("test" in filepath or "seaborn/_core.py" == filepath):
            print("Could not find file content:", filepath)
            continue
