...
class OllamaChatDecoder(DecoderBase):
    ...
    def codegen(
        ...
        trajs = []
        for _ in range(num_samples):
            # The Ollama API does not support batching like OpenAI.
            ...
            ret = self._clean_response(ret.message.content)
                ...
                trajs.append(
                    {
                        "response": ret,
                        "usage": {
                            # Ollama's local API does not provide token usage info in the same format
                            "completion_tokens": 0,
                            "prompt_tokens": 0,
                        },
                    }
